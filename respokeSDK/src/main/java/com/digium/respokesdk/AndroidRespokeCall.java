 ...
 */
public class AndroidRespokeCall extends RespokeCall{
    ...
    public AndroidRespokeCall(RespokeSignalingChannel channel) {
        commonConstructor(channel);
    }


    public AndroidRespokeCall(RespokeSignalingChannel channel, RespokeEndpoint newEndpoint, boolean directConnectionOnly) {
        commonConstructor(channel);
    ...
    public AndroidRespokeCall(RespokeSignalingChannel channel, JSONObject sdp, String newSessionID, String newConnectionID, RespokeEndpoint newEndpoint, boolean directConnectionOnly, Date newTimestamp) {
        commonConstructor(channel);
        ...
        audioOnly = !AndroidRespokeCall.sdpHasVideo(sdp);
                                ...
                                if (null != hangupListener) {
                                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                                        public void run() {
                                            ...
                                            if (null != listener) {
                                                listener.onHangup(AndroidRespokeCall.this);
                                            }
                                        }
                                    });
                                }
                ...
                public void onSuccess() {
                    ...
                    if (null != listenerReference) {
                        ...
                        if (null != listener) {
                            new Handler(Looper.getMainLooper()).post(new Runnable() {
                                public void run() {
                                    listener.onConnected(AndroidRespokeCall.this);
                                }
                            });
                        }
                    }
            ...
            if (newState == PeerConnection.IceConnectionState.CONNECTED) {
                ...
                if (null != listenerReference) {
                    ...
                    if (null != listener) {
                        new Handler(Looper.getMainLooper()).post(new Runnable() {
                            public void run() {
                                listener.onError("ICE Connection failed!", AndroidRespokeCall.this);
                                listener.onHangup(AndroidRespokeCall.this);
                            }
                        });
                    }
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            public void run() {
                if (null != listenerReference) {
                    Listener listener = listenerReference.get();
                    if (null != listener) {
                        listener.onError(errorMessage, AndroidRespokeCall.this);
                    }
                }
            }
        });
